...
const Dashboard = () => {
  ...
  const getNewData = (startingDate, endingDate) => {
    let _url = `Dashboard/TopStatistics?candidateId=${sessionStorage.getItem(
          ...
          {profileProgress !== 100 && (
            <div className={Style.profileUpdateBanner}>
              ...
              <div className={Style.textContainer}>
                <p>
                  {profileProgress}% profile completed. Please update your profile and get better responses from companies.
                </p>
              </div>
              <div>
                <Button
              ...
              <Col lg={15} xs={24}>
                <OverviewCharts
                  allJobApplicationsCount={
                    data?.userDashboard?.allJobsApplicationCount
                  }
                  approvedApplicationsCount={
                    data?.userDashboard?.allApprovedApplicationsCount
                  }
                  rejectedApplicationsCount={
                    data?.userDashboard?.allRejectedApplicationsCount
                  }
                  shortListedApplicationsCount={
                    data?.userDashboard?.allShortListedApplicationsCount
                  }
                  applicationsRateGraph={
                    data?.userDashboard?.applicationsRateGraph
                  }
                ...
                <Row
                  className={[Style.dboardBttomCols, Style.pt30]}
                  gutter={30}
                >
                  <Col lg={8} md={8} xs={24} className={Style.col}>
                    <ApprovedJobs
                      approvedJobs={
                        data?.userDashboard?.allApprovedApplications
                      }
                    />
                  </Col>
                  <Col
                    ...
                    md={8}
                    xs={24}
                    className={[Style.col, Style.ptSm30]}
                  >
                    <ShortListedJobs
                      shortListedJobs={
                        data?.userDashboard?.allShortListedApplications
                      }
                    />
                  </Col>
                  <Col
                    ...
                    md={8}
                    xs={24}
                    className={[Style.col, Style.ptSm30, Style.pbSm30]}
                  >
                    <RejectedJobs
                      rejectedJobs={
                        data?.userDashboard?.allRejectedApplications
                      }
                    />
                  </Col>
                </Row>
