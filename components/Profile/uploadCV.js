...
import { UploadCV_atom } from "../../stores/atoms/profile_atom";
...
const UploadCV = (props) => {
  ...
  const inner_props = {
    ...
    onChange(info) {
      if (info.file.name.split(".").pop() !== "pdf" && info.file.name.split(".").pop() !== "docx") {
        addToast("Only PDF and docx files are allowed", {
          appearance: "error",
          autoDismiss: true,
        });
        return false;
