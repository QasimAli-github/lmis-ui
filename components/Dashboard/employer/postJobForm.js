...
import {
  ...
  FetchSelectLookup,
} from "../../../shared/fetch_select_lookup";
...
const PostJobForm = ({
  ...
  useEffect(() => {
    if (jobObjToEdit) {
      ...
      form.setFieldsValue({
        ...
        skillsRequired: jobObjToEdit.skillsRequired
          ? jobObjToEdit.skillsRequired.split(",").map((x) => +x)
          : null,
  ...
  const {
    ...
    error,
    data: jobCategories,
  } = useQuery("getAllLookups", () => getLookups(12));
  ...
  return (
    <div className={Style.postJobForm}>
      ...
      >
        ...
        <Form.Item
          label={t("no_of_vacancies")}
          rules={[{ required: true }]}
          name="noOfVacancy"
        >
          <Input
            placeholder={t("no_of_vacancies")}
            type="number"
            size="large"
          />
        </Form.Item>
        ...
        >
          <Select
            ...
            style={{ width: "100%" }}
            size="large"
            placeholder="Enter Jobs Category, seperated with comma"
          >
            {jobCategories &&
              jobCategories.map((cetegory) => {
                return (
                  <Option key={Math.random()} value={cetegory.lookUpValueId}>
                    {language === "en"
                      ? cetegory.lookUpValue
                      : cetegory.lookUpValue_Ar}
                  </Option>
                );
              })}
          </Select>
